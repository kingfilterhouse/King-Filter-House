...
async function issueRefund(accountId) {
    try {
        ...
        // Create a special payment record for the refund
        // Note: We mark this with is_refund flag but use negative amount
        const refundPayment = {
            ...
            notes: `MANUAL REFUND: Refund issued for account overpayment (negative balance cleared)`
        };
        
        const result = await window.StorageModule.saveData('payments', refundPayment);
        ...
        if (result.success) {
            ...
            window.log('✅ Refund issued:', refundPayment);
        } else {
