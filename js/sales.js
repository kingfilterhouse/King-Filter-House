...
async function loadSales() {
    try {
        ...
        if (result.success) {
            ...
            console.log('✅ Loaded', salesListData.length, 'sales');
...
async function applyFilters() {
    try {
        ...
        console.log('🔍 Applying filters:', { searchTerm, statusFilter, dateFrom, dateTo });
        ...
        console.log('✅ Filtered:', filteredSalesData.length, 'of', salesListData.length, 'sales');
...
async function renderSales() {
    ...
    if (!salesTableBody) {
        console.warn('⚠️ Sales table body element not found');
        return;
    }
    ...
    // Load sale items for counting
    const saleItemsPromises = filteredSalesData.map(async (sale) => {
        const itemsResult = await window.StorageModule.getSaleItems(sale.id);
        return { saleId: sale.id, items: itemsResult.data || [] };
    });

    const saleItemsData = await Promise.all(saleItemsPromises);
    const saleItemsMap = {};
    saleItemsData.forEach(({ saleId, items }) => {
        saleItemsMap[saleId] = items;
    });
    ...
    salesTableBody.innerHTML = filteredSalesData.map(sale => {
        ...
        return `
            <tr>
                ...
                <td>
                    <span class="sale-status-badge ${statusClass}">${sale.payment_status || 'unpaid'}</span>
                </td>
                <td>
...
function updateSummaryStats() {
    ...
    console.log('📊 Stats (from ALL data):', { totalSales, totalRevenue, pendingAmount, avgValue });
...
window.viewSaleDetails = async function(saleId) {
    try {
        ...
        if (!saleDetailModal || !saleDetailContent) {
            console.error('❌ Sale detail modal elements not found');
        ...
        console.log('📦 Loaded sale items:', items.length, items);
...
window.editSale = async function(saleId) {
    try {
        ...
        if (!editSaleModal) {
            console.error('❌ Edit sale modal not found in HTML!');
...
window.saveEditedSale = async function() {
    ...
    try {
        ...
        } catch (e) { console.warn('⚠️ Could not fetch return items for stock calc:', e); }
    ...
    if (confirmDeleteSaleBtn) {
        confirmDeleteSaleBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                console.log('📦 Restoring stock for sale items:', items.length);
                ...
                if (saleReturnsResult.data && saleReturnsResult.data.length > 0) {
                    ...
                    console.log('✅ Deleted', saleReturnsResult.data.length, 'associated return(s)');
                }
...
console.log('✅ Sales Module Loaded (COMPLETE FIX)');
